            ...
            string processArgs, Action<IMonitorNotification>? processNotification, CancellationToken ct)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    // job name check
                    { Assert.That(resp is GetJobIdResp { JobId: var j } && j.IsInvalid()); }
                    ...
                    // job name check
                    { Assert.That(resp is GetJobIdResp { JobId: var j } && j == jobId); }
            ...
            Assert.That(await Task.WhenAny(monitorTask, Task.Delay(maxMonitorIdleTime * 2, ct)), Is.EqualTo(monitorTask));
